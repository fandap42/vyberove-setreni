...
mzdy_vector <- mzdy_data$Mzda

population_mzdy <- rep(mzdy_vector, times = R)

N_mzdy <- length(population_mzdy)
mean_population_mzdy <- mean(population_mzdy)
var_population_mzdy <- var(population_mzdy)
S2_mzdy <- var_population_mzdy * (N_mzdy - 1) / N_mzdy

cat(paste("Velikost základního souboru (N):", N_mzdy, "\n"))
cat(paste("Populační průměr (Y_bar):", round(mean_population_mzdy, 2), "Kč\n"))
cat(paste("Populační rozptyl (S^2):", round(S2_mzdy, 2), "\n"))
```
...
$$ n = \frac{1}{\frac{\Delta^2}{z_{1-\alpha/2}^2 \cdot S^2} + \frac{1}{N}} $$
...
for (conf in confidence_levels) {
  for (delta in deltas_mean) {
    n <- get_sample_size_mean(N_mzdy, S2_mzdy, delta, conf)
...
Maximální počet zaměstnanců, které máme k dispozici, je `100 * R = 100 * 10 = 1000`.
Z tabulky je patrné, že s našimi zdroji jsme schopni dosáhnout přesnosti s šířkou intervalu 2000 Kč při všech úrovních spolehlivosti a přesnosti 1500 Kč při 90% a 95% spolehlivosti. Nejpřesnější odhad (šířka 1000 Kč) je při vyšších spolehlivosti 99 % již mimo naše možnosti.
...
conf_level_sim <- 0.95
delta_sim <- 750
n_sim <- get_sample_size_mean(N_mzdy, S2_mzdy, delta_sim, conf_level_sim)
...
for (i in 1:num_simulations) {
  sample_data <- sample(population_mzdy, n_sim, replace = FALSE)
  
  sample_mean <- mean(sample_data)
  sample_var <- var(sample_data)
  
  se <- sqrt((1 - n_sim / N_mzdy) * sample_var / n_sim)
  ...
  coverage[i] <- (lower_bound <= mean_population_mzdy && upper_bound >= mean_population_mzdy)
}

avg_interval_width <- mean(interval_widths)
coverage_rate <- mean(coverage)

cat(paste("Teoretická šířka intervalu:", 2 * delta_sim, "Kč\n"))
cat(paste("Průměrná šířka intervalu ze simulací:", round(avg_interval_width, 2), "Kč\n"))
cat(paste("Teoretická spolehlivost:", conf_level_sim * 100, " %\n"))
cat(paste("Dosažená spolehlivost (pokrytí):", coverage_rate * 100, " %\n"))
```

Průměrná šířka intervalu ze simulací se velmi blíží teoretické požadované šířce (1500 Kč). Dosažená spolehlivost (míra pokrytí) je také velmi blízko teoretické 95% spolehlivosti. To potvrzuje správnost našeho teoretického výpočtu rozsahu výběru.
...
population_levels <- data.frame(
  Uroven = c("Celostátní", "Kraj (velký)", "Kraj (malý)", "Okresní"),
  N = c(5000000, 600000, 250000, 25000) 
)

for (i in 1:nrow(population_levels)) {
  level_name <- population_levels$Uroven[i]
...
for (i in 1:num_simulations_prop) {
  sample_data_prop <- sample(population_prop, n_sim_prop, replace = FALSE)
...
cat(paste("Teoretická šířka intervalu:", 2 * delta_sim_prop * 100, " %\n"))
cat(paste("Průměrná šířka intervalu ze simulací:", round(avg_interval_width_prop * 100, 3), " %\n"))
cat(paste("Teoretická spolehlivost:", conf_level_sim_prop * 100, " %\n"))
cat(paste("Dosažená spolehlivost (pokrytí):", coverage_rate_prop * 100, " %\n"))
```

### iii) Bonusová úloha

Simulací odhadneme pravděpodobnosti správného úsudku na základě bodového odhadu `p_hat` při `n=1000`. Skutečný podíl voličů je `P = 4 %`.
...
for (i in 1:num_sim_bonus) {
  sample_bonus <- sample(population_bonus_true, n_bonus, replace = FALSE)
  p_hats[i] <- mean(sample_bonus)
}

# a) P(p_hat >= 0.03) -> narok na statni podporu
prob_support <- mean(p_hats >= 0.03)
cat(paste("a) Odhadovaná P, že hnutí získá podporu (p_hat >= 3%):", round(prob_support, 4), "\n"))
...
cat(paste("b) Odhadovaná P, že správně usoudíme vstup do parlamentu (p_hat >= 5% | P=6%):", round(prob_parliament_if_in, 4), "\n"))

# c) P(p_hat < 0.05 | P < 0.05) -> nedostane se do parlamentu
#    Musime provest novou simulaci s P < 0.05, zvolme napr. P = 4%
P_under_5 <- 0.04
population_bonus_under_5 <- rbinom(N_bonus, 1, P_under_5)
p_hats_under_5 <- numeric(num_sim_bonus)

for (i in 1:num_sim_bonus) {
  sample_bonus_under_5 <- sample(population_bonus_under_5, n_bonus, replace = FALSE)
  p_hats_under_5[i] <- mean(sample_bonus_under_5)
}
prob_no_parliament_if_out <- mean(p_hats_under_5 < 0.05)
cat(paste("c) Odhadovaná P, že správně usoudíme nevstup do parlamentu (p_hat < 5% | P=4%):", round(prob_no_parliament_if_out, 4), "\n"))
```
