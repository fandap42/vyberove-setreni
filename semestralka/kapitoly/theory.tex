...
Nechť $X_1, X_2, \dots, X_n$ je náhodný výběr, tj. posloupnost nezávislých stejně rozdělených (i.i.d.) náhodných veličin, z rozdělení se spojitou distribuční funkcí $F(x)$ a hustotou pravděpodobnosti $f(x)$. Předpokládáme, že $F$ je absolutně spojitá.
...
Výběrový kvantil $\hat{q}_p$ je definován pomocí empirické distribuční funkce $\hat{F}_n(x)$, která je dána předpisem:
\begin{equation}
    \hat{F}_n(x) = \frac{1}{n} \sum_{i=1}^n \mathbb{I}(X_i \le x),
\end{equation}
kde $\mathbb{I}(\cdot)$ je indikátorová funkce. Výběrový kvantil je pak definován jako \parencite{Hyndman1996}:
\begin{equation}
